...
cart.forEach((productInCart) => {
  fetchArray.push(
    fetch("http://localhost:3000/api/products/" + productInCart.id).then(
...
function addProduct(product, i) {
  ...
  cartItemSettingsQuantity.setAttribute(
    "class",
  ...
  cartItemSettingsQuantity.append(
    cartItemSettingsQuantityText,
    cartItemSettingsInput
  );
  ...
  cartItemSettingsDelete.addEventListener("click", () => {
    ...
    let closestCartItemIndex = cart.findIndex(
      (cartItem) =>
        cartItem.id == closestCartItem.getAttribute("data-id") &&
        cartItem.color == closestCartItem.getAttribute("data-color")
    );
...
let calculateTotalPrice = () => {
  ...
  for (let i = 0; i < cart.length; i++) {
    ...
    totalPriceVariable =
      totalPriceVariable + cart[i].price * Number(cart[i].quantity);
...
orderButton.addEventListener("click", (event) => {
  ...
  if (
    ...
    textRegEx.test(lastName.value) &&
    addressRegEx.test(address.value) &&
    textRegEx.test(city.value) &&
    ...
    fetch("http://localhost:3000/api/products/order", postContact)
      ...
      .then((data) => {
        console.log(
          data,
          "http://127.0.0.1:5500/front/html/confirmation.html/" + data.orderId
        );
